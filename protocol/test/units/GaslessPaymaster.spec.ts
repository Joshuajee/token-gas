...
describe("GaslessPaymaster ", function () {
  ...
  async function deployAndSupplyLiquidity() {
    ...
    await deployed.GaslessPaymaster.write.deposit(
  ...
  async function transfer(deployed: any, maxFee = parseEther("2.5", "wei")) {
    ...
    const recipientBalInitial = await USDC.read.balanceOf([
      user3.account.address,
    ]);

    const initialTokenBalOfProtocol = await USDC.read.balanceOf([
      GaslessPaymaster.address,
    ]);
    ...
    const callerInitialBalance = await publicClient.getBalance({
    ...
    expect(await USDC.read.balanceOf([user3.account.address])).to.be.equal(
      recipientBalInitial + amount
    );
    ...
    expect(
      Number(
        (await publicClient.getBalance({ address: caller.account.address })) -
          callerInitialBalance
      )
    ).to.be.gt(Number(await GaslessPaymaster.read.callerFeeAmountInEther()));
  ...
  async function swapOnPancake(
    deployed: any,
    maxFee = parseEther("2.5", "wei")
  ) {
    const {
      GaslessPaymaster,
      user1,
    ...
    const iniatlSwaperbalance = await USDC.read.balanceOf([
      user1.account.address,
    ]);
    ...
    // Recipient Balance should be equal to zero before Transfer
    const recipientBalInitial = await DAI.read.balanceOf([
      user3.account.address,
    ]);
    ...
    // Recipient Balance should be equal to amount before after
    expect(
      Number(await DAI.read.balanceOf([user3.account.address]))
    ).to.be.closeTo(
      Number(recipientBalInitial + amountIn),
      Number(parseEther("1000", "wei"))
    );

    // Amount sent should be deducted from Sender Balance
    expect(
      Number(await USDC.read.balanceOf([user1.account.address]))
    ).to.be.lessThan(Number(iniatlSwaperbalance));
    ...
    // check if the contract received the fee
    expect(
      Number(await USDC.read.balanceOf([GaslessPaymaster.address]))
    ).to.be.greaterThan(Number(initialTokenBalOfProtocol));
    ...
    describe("Adding Liquidity",  function () {
        ...
        it("Should Add Liquidity and Mint LP tokens to receiver", async ( ) => {
            ...
            const { GaslessPaymaster, user1, user2, publicClient  } = await loadFixture(deploy)
  ...
  describe("Deployment", function () {
    it("Should deploy", async () => {
      ...
      expect(await GaslessPaymaster.read.token()).to.be.equal(
        checksumAddress(USDC.address)
      );

      expect(await GaslessPaymaster.read.owner()).to.be.equal(
  ...
  describe("Adding Liquidity", function () {
    it("Should Add Liquidity and Mint LP tokens to receiver", async () => {
      ...
      expect(
        await GaslessPaymaster.read.balanceOf([user1.account.address])
      ).to.be.equal(value - 1n);
      ...
      expect(
        await GaslessPaymaster.read.balanceOf([user2.account.address])
      ).to.be.equal(value - 1n);
    });
  });
  ...
  describe("Transfers", function () {
    it("Should be able to transfer tokens", async () => {
      const {
        ...
        domainPermit2,
        USDC,
      } = await loadFixture(deployAndSupplyLiquidity);
      ...
      expect(
        Number(
          (await publicClient.getBalance({ address: user1.account.address })) -
            callerInitialBalance
        )
      ).to.be.gt(Number(await GaslessPaymaster.read.callerFeeAmountInEther()));
    ...
    describe("Redeeming Tokens",  function () {
        ...
        it("Should be able to correctly estimate Redeem", async () => {

            const deployed = await loadFixture(deployAndSupplyLiquidity)

            const balLpToken = await deployed.GaslessPaymaster.read.balanceOf([deployed.user1.account.address])

            expect(await deployed.GaslessPaymaster.read.getFundShare([balLpToken])).to.deep.equal([
                0n, balLpToken
            ])
    ...
    it("Should be able pay Transaction with estimated fee", async () => {
      ...
      const maxFee = await deployed.GaslessPaymaster.read.estimateFees([
        0,
        10365794880n,
      ]);
  ...
  describe("Withdrawals", function () {
    it("Should be able to correctly estimate Withdrawals", async () => {
      ...
      expect(
        await deployed.GaslessPaymaster.read.getFundShare([deployed.value])
      ).to.deep.equal([0n, deployed.value]);

      await transfer(deployed);

      const contractUSDBal = await deployed.USDC.read.balanceOf([
        deployed.GaslessPaymaster.address,
      ]);

      const smallAmount = parseEther("0.0001", "wei");

      expect(
        await deployed.GaslessPaymaster.read.getFundShare([smallAmount])
      ).to.deep.equal([
        await calculatePrice(smallAmount, usdcPriceFeeds, bnbPriceFeeds),
        0n,
      ]);
      ...
      expect(
        ...
        contractUSDBal - 1n,
        deployed.value -
          (await calculatePrice(contractUSDBal, bnbPriceFeeds, usdcPriceFeeds)),
      ]);
    });
    ...
    it("Should be able to withdraw BNB, when no transfer has been made - partial withdrawal", async () => {
      ...
      const initialLPBal = await deployed.GaslessPaymaster.read.balanceOf([
        deployed.user1.account.address,
      ]);

      const initialBal = await deployed.publicClient.getBalance({
        address: deployed.GaslessPaymaster.address,
      });

      const amount = (initialLPBal + 1n) / 2n;

            await deployed.GaslessPaymaster.write.redeem([
                amount, 
                deployed.user1.account.address, 
                deployed.user1.account.address
            ])
        ...
        it("Should only withdraw usdc", async ( ) => {
      console.log("------------------------------------------------");
      ...
      expect(
        await deployed.GaslessPaymaster.read.balanceOf([
          deployed.user1.account.address,
        ])
      ).to.be.equal(initialLPBal - amount);

      expect(
        await deployed.publicClient.getBalance({
          address: deployed.GaslessPaymaster.address,
        })
      ).to.be.equal(initialBal - amount);
    });
    ...
    //     const deployed = await loadFixture(deployAndSupplyLiquidity)
            ...
            const user = deployed.user1.account.address

            const balLpToken = await deployed.GaslessPaymaster.read.balanceOf([user])
    ...
    //     await transfer(deployed)
            ...
            const userUSDBal = (await deployed.USDC.read.balanceOf([deployed.user1.account.address]))

            const contractUSDBal = (await deployed.USDC.read.balanceOf([deployed.GaslessPaymaster.address]))
    //     const contractUSDBal = (await deployed.USDC.read.balanceOf([deployed.GaslessPaymaster.address]))

            const smallAmount = parseEther("0.1", "wei")
    //     const smallAmount = parseEther("0.0001", "wei")

            const redeemShare = await deployed.GaslessPaymaster.read.getFundShare([smallAmount])

            console.log(await deployed.GaslessPaymaster.read.getFundShare([smallAmount]))

            console.log({redeemShare, contractUSDBal, userUSDBal})

            await deployed.GaslessPaymaster.write.redeem([smallAmount, user, user])

            expect(await deployed.USDC.read.balanceOf([deployed.user1.account.address])).to.be.equal(userUSDBal + redeemShare[0])
    ...
    //     ])
        ...
        it("Should be able to withdraw USDC and BNB", async ( ) => {
            ...
            await deployed.GaslessPaymaster.write.redeem([balLpToken, user, user])
  ...
  describe("pancakeSwap", function () {
    it("Should Be able to swap tokens", async () => {
      const {
        ...
        domainPermit,
        domainPermit2,
      } = await deployAndFundPancakeSwapPools();
      ...
      const amountIn = parseEther("1000", "wei");
            ...
            const path = encodePath([usdcAddress, daiAddress], [FeeAmount.HIGH])
        ...
        it("Should Be able to swap tokens for BNB", async ( ) => {
            ...
            const amountIn = parseEther("10", "wei")
            ...
            const path = encodePath([usdcAddress, zeroAddress], [FeeAmount.HIGH])
      ...
      await GaslessPaymaster.write.swapPancakeSwapGasless([
        permitData,
        swapData,
      ]);
