...
describe("GaslessPaymaster ", function () {
  ...
  describe("Adding Liquidity", function () {
    it("Should Add Liquidity and Mint LP tokens to receiver", async () => {
      const { GaslessPaymaster, user1, user2, publicClient } =
  ...
  describe("Redeeming Tokens", function () {
    it("Should be able to correctly estimate Redeem", async () => {
      ...
      const balLpToken = await deployed.GaslessPaymaster.read.balanceOf([
        deployed.user1.account.address,
      ]);

      expect(
        await deployed.GaslessPaymaster.read.getFundShare([balLpToken])
      ).to.deep.equal([0n, balLpToken]);
    ...
    it("Should only withdraw usdc", async () => {
      ...
      const userUSDBal = await deployed.USDC.read.balanceOf([
        deployed.user1.account.address,
      ]);
      ...
      const redeemShare = await deployed.GaslessPaymaster.read.getFundShare([
        smallAmount,
      ]);
      ...
      expect(
        await deployed.USDC.read.balanceOf([deployed.user1.account.address])
      ).to.be.equal(userUSDBal + redeemShare[0]);
